...
Technical Requirements:
  ...
  - Framework: Any JS framework (Angular or React preferred) // not developed 
  - State Management: Native framework tools (component state, reactive forms, etc.) // not developed
...
3. **CSS -> BootStrap 5**
   - Body: lightgrey background that spans all the page, elements centered
   - Table & Forms: postioned side by side (on the same row), with spacing between them
     1. Table Behavior: takes all the horizontal space, content fully displayed
     2. Table Appearance: borders, alternate colours row for readability 
     3. Forms: fixed column (4 rows), spacing between Insert and Update
